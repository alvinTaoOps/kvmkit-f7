...
void ConsoleInit(void)
{
	...
	ConsoleIoInit();
	ConsoleIoSendString("KVMKit Console Init!");
...
eCommandResult_T ConsoleReceiveParamString(const char * buffer, const uint8_t parameterNumber, uint32_t * lenArg, char * args)
{
	...
	while ( ( LF_CHAR != charVal ) && ( CR_CHAR != charVal )
			&& ( PARAMETER_SEPARATER != charVal ) && ( i < maxArgLen ) )
	{
...
eCommandResult_T ConsoleReceiveParamInt16(const char * buffer, const uint8_t parameterNumber, int16_t* parameterInt)
{
	...
	if ( COMMAND_PARAMETER_END == result ) { // Return if the requested parameterNumber isn't found
		return result;
	}
...
/**
 * Returns the (p_index)th parameter (counting the command string as index 0) as an unsigned, 8-bit, integer
 */
uint8_t GetParamUInt8(const char buffer[], uint8_t p_index, uint8_t * param)
{
	int16_t parameterInt;
	eCommandResult_T result;
	result = ConsoleReceiveParamInt16(buffer, p_index, &parameterInt);
	if ( COMMAND_SUCCESS == result )
	{
		if ( parameterInt < 0 ){
			return COMMAND_PARAMETER_ERROR;
		}
		*param = (uint8_t) parameterInt & 0xFFu;
	}
	return result;
}

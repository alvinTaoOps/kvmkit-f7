...
static const sConsoleCommandTable_T mConsoleCommandTable[] =
{
    ...
    {"cpk", &ConsoleCommandPutOneKey, HELP("Send one keypress")}, // TODO change this up to accept directly a usb hid report
...
static eCommandResult_T ConsoleCommandPutOneKey(const char buffer[])
{
	...
	result = ConsoleReceiveParamString(buffer, 1, &argLen, cmdArg);
	if ( COMMAND_SUCCESS == result )
	{
		InsertCharacter(cmdArg[0]);
		ConsoleIoSendString(STR_ENDLINE);
	}
	return result;
}
